    ...
    def process_dataset(self, dataset_config):
        ...
        try:
            ...
            for zip_file in downloaded_files:
                # Special handling for AVM dataset - skip extract/filter, upload directly
                if dataset_name == "AVM":
                    self.logger.info(f"=== AVM Direct Upload Process ===")
                    self.logger.info(f"Processing AVM file: {zip_file.name}")
                    self.logger.info(f"Skipping extraction and filtering")
                    self.logger.info(f"Adding to upload queue: {zip_file} (size={zip_file.stat().st_size} bytes)")
                    all_filtered_zips.append(zip_file)
                    continue
            ...
            if all_filtered_zips:
                ...
                self.logger.info(f"=== FTP Upload Configuration ===")
                self.logger.info(f"Dataset: {dataset_name}")
                self.logger.info(f"Upload folder: {ftp_folder}")
                self.logger.info(f"FTP upload enabled: {dataset_config.get('filter_ftp_upload', False)}")
                self.logger.info(f"FTP action: {dataset_config.get('filter_ftp_action', 'copy')}")
                self.logger.info(f"Files to upload: {len(all_filtered_zips)}")
                ftp_map = {}
                if dataset_config.get('filter_ftp_upload', False) and ftp_folder and self.ftp_uploader:
                    try:
                        self.logger.info("=== Starting FTP Upload Process ===")
                        ...
                        for r in ftp_results:
                            ...
                            if ok:
                                ...
                                self.logger.info(f"✓ Successfully uploaded to FTP: {Path(path).name} -> {ftp_folder}")
                            else:
                                failed_uploads += 1
                                self.logger.warning(f"✗ Failed FTP upload: {Path(path).name}")
                        
                        self.logger.info("=== FTP Upload Summary ===")
                        self.logger.info(f"Total files: {len(ftp_results)}")
